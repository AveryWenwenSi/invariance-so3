...
from pgd_attack import LinfPGDAttack, SpatialPGDAttack, SpatialGDBatchAttack
EPS = 1e-10
...
class SpatialAttack:
    def __init__(self, model, config, method=None, worstofk=None,
        ...
        if config.use_spatial:
            ...
            if self.method == 'grid':
                ...
                self.fo_attack = SpatialGDBatchAttack(
                    model, config, fo_epsilon, fo_step_size, fo_num_steps)
            else:
    ...
    def kl_reg_loss(self, dist_nat, dist_adv):
        ...
        prob_adv = special.softmax(dist_adv, axis=1) + epsilon
        prob_nat = special.softmax(dist_nat, axis=1) + epsilon

        # scipy.stats.entropy calculate the KL divergence (although it's called entropy)
        return stats.entropy(np.transpose(prob_nat), np.transpose(prob_adv))
