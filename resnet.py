...
class Model(object):
  ...
  def _build_model(self, config, filters, num_ids, differentiable=False,
    ...
    with tf.variable_scope('input'):
      ...
      # SL3 change 
      self.adv_idx = tf.placeholder(tf.int32, [None])
      self.cce_idx = tf.placeholder(tf.int32, [None])
      ...
      if not differentiable:
        ...
        # ones = tf.ones(shape=tf.shape(rot1))
        # zeros = tf.zeros(shape=tf.shape(rot1))
        ...
        # change 2.1
        theta=so3.exp(self.transform )
        # theta=se3_tf.exp(self.transform * np.pi / 180)
        x = transformer(x, theta, (64, 64))
      else:
        # change 2.2
        # theta=so3.exp(self.transform * np.pi / 180 )
        theta=so3.exp(self.transform )
        # theta=se3_tf.exp(self.transform * np.pi / 180)
        # theta = tf.matrix_diag(diagonal)
        # theta = tf.linalg.expm(theta)
