...
def train(config='configs/std.json',
          save_root_path='./',
    ...
    if use_reg == None:
        # use_reg = config.training.use_reg
    ...
    learning_rate = tf.train.cosine_decay_restarts(
            0.1, tf.cast(global_step, tf.int32), 1200, t_mul=2, m_mul=0.5,
        )
    ...
    attack_eval_random = SpatialAttackEval(model, config.attack, 'random', 10,
